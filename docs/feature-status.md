...
```
Tests       1652 passed (251 engine regression, 31 engine unit, 20 fund,
                         ...
                         12 workbook reading, 6 export cell formatting, 3 covenant formatting,
                         5 workbook import,
                         ...
                         9 entitlements via the API, 6 organization export,
                         ...
                         6 local object storage, 8 documents, 3 updating a property, 8 dashboards,
                         6 lease options through the API,
                         4 the shared numeric form-field validator,
                         2 property spaces batch atomicity)
Browser     262 passed  (3 sign-in, 5 underwriting and the virtualised grid,
                         ...
                         2 sign-in's own field-invalid marking)
...
| CREOS handoff import: SiteIntel (Phase 5) + MarketSignal (Phase 6) | Tested | Reads a `creos-handoff-v1` `.json` file downloaded from CREOS SiteIntel's or CREOS MarketSignal's own "Send to Underwrite" action (independent applications — no shared package or backend). Fails closed on a malformed file or a governance violation — a non-user observation/assumption claiming `accepted`/`overridden`/`rejected` instead of `proposed` is refused outright, re-checked here rather than trusted from the sender. A valid handoff is translated into a `cre-assumption-import` v1 document and flows through the exact same paste → analyze → accept-by-hand review any other import uses; nothing is written automatically. SiteIntel facts are always informational (`siteIntel.*` targets, "Unsupported") — it reports observed facts (assessed value, zoning, a prior sale) and explicitly refuses to supply things like acquisition price, so there is no real model field one could correctly overwrite. MarketSignal is different: 3 of its 15 forecast types (vacancy, exit cap rate, discount rate) route to real, directly-applicable targets (`vacancy.generalVacancyRate`, `valuation.terminalCapRate`, `valuation.discountRate`) and analyze as new/changed like any other proposal — the other 12 (rent growth, leasing-profile fields, ...) need a model-specific collection row MarketSignal can't know at export time, so those stay informational (`marketSignal.*`) rather than sending a partially-resolved path that would misrepresent itself as real. 48 domain-model tests. See `docs/creos-ids.md` and, in the CREOS Enterprise repository, `docs/HANDOFF_DESIGN.md` |
