...
```
Tests       1537 passed (251 engine regression, 31 engine unit, 20 fund,
                         ...
                         27 the SiteIntel handoff import,
...
| CREOS SiteIntel handoff import (Phase 5) | Tested | Reads a `creos-handoff-v1` `.json` file downloaded from CREOS SiteIntel's own "Send to Underwrite" action (an independent application — no shared package or backend between them). Fails closed on a malformed file or a governance violation — a non-user observation claiming `accepted`/`overridden`/`rejected` instead of `proposed` is refused outright, re-checked here rather than trusted from the sender. A valid handoff is translated into a `cre-assumption-import` v1 document and flows through the exact same paste → analyze → accept-by-hand review any other import uses; nothing is written automatically. Every translated fact is deliberately informational (`siteIntel.*` targets, always "Unsupported") rather than a live underwriting input — SiteIntel reports observed facts (assessed value, zoning, a prior sale) and explicitly refuses to supply things like acquisition price, so there is no real model field one of its facts could correctly overwrite. 27 domain-model tests. See `docs/creos-ids.md` and, in the CREOS Enterprise repository, `docs/HANDOFF_DESIGN.md` |
