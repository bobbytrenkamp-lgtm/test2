...
| 13 | Organization assumption libraries | **Partial** | Three of the four planned families are reusable and traceable. Growth curves: `growth_curve_templates` (migration 0020) plus `growth_curves.source_template_code`/`source_template_name` (migration 0021). Market leasing profiles: `market_leasing_profile_templates` plus `market_leasing_profiles.source_template_code`/`source_template_name` (both migration 0022). Operating expenses: `operating_expense_templates` plus `operating_expenses.source_template_code`/`source_template_name` (both migration 0023). All three follow the identical pattern: an organization-scoped, code-addressable template table (`GET`/`PUT`/`DELETE /organizations/:id/<family>-templates/:code`, gated on `model:write`); a `TemplateLibraryCard` on the Organization admin screen (one generic component, parameterized per family — see `apps/web/src/pages/Organization.tsx`); a "Start from library" picker on the model's own collection editor that seeds a new row's fields and stamps the provenance snapshot, never a live reference, so editing or deleting a library entry afterward neither breaks nor silently changes a model that already applied it. Starting an expense from the library now opens the collection's own structured `RecordEditor` directly (`AssumptionsTab.tsx`'s `beginFromTemplate`, generalized alongside this feature) rather than forcing the raw JSON view market leasing was limited to — provenance still survives whichever form the analyst saves from, via the `pendingSourceTemplate` mechanism merged into the shared save call regardless of path. Acquisition/disposition/debt templates are next; those are scalar fields on `models` (sale cost %, terminal cap rate, acquisition costs) rather than a collection, so the code-addressable table pattern above may not transfer directly and needs its own design pass rather than a mechanical copy |
