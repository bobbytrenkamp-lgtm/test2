...
| 13 | Organization assumption libraries | **Partial** | Growth curves are now reusable: `growth_curve_templates` (migration 0020) holds a named, organization-scoped library (`GET`/`PUT`/`DELETE /organizations/:id/growth-curve-templates/:code`, gated on `model:write`), managed from the "Growth curve library" card on the Organization admin screen, and any model's own growth-curve editor can seed a new row from one ("Start from library"). Deliberately not a live reference — applying a template only fills a new row's fields; a model keeps its own copy from that point, matching how every other assumption here is a value the model owns once written. Traceable, not just copied: `growth_curves.source_template_code`/`source_template_name` (migration 0021) snapshot which library entry a curve started from and what it was called at that moment — a plain text record, not a foreign key, so renaming or deleting the template afterward neither breaks nor silently changes the model's own copy. Market-leasing profiles, expense categories, and appropriate acquisition/disposition and debt assumptions are next, extending this exact pattern (organization-scoped template table, code-addressable upsert, copy-not-reference apply, provenance snapshot) rather than inventing a separate approach per assumption family |
