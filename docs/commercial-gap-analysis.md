...
| 13 | Organization assumption libraries | **Partial** | Growth curves are now reusable: `growth_curve_templates` (migration 0020) holds a named, organization-scoped library (`GET`/`PUT`/`DELETE /organizations/:id/growth-curve-templates/:code`, gated on `model:write`), managed from the new "Growth curve library" card on the Organization admin screen, and any model's own growth-curve editor can seed a new row from one ("Start from library"). Deliberately not a live reference — applying a template only fills a new row's fields; a model keeps its own copy from that point, matching how every other assumption here is a value the model owns once written. Market-leasing profiles and expense categories are still model-scoped only; the same reusable-library pattern extends to them next |
