...
| Portfolio aggregation, including a dynamic filter shaped wrong rejected with a 400 rather than reaching Postgres as an unhandled array-cast error | `tests/portfolios.test.ts` | 8 | Yes |
| Optimistic locking, leases, models and collections; ten concurrent "create version" requests on one model never colliding on `model_versions`' own `UNIQUE (model_id, version_number)`; and a model's nullish fields (notes, rate overrides, the gross sale price override) actually clearing on an explicit `null` rather than `COALESCE` silently keeping the old value | `tests/lease-concurrency.test.ts` | 19 | Yes |
| Recovery pools through the API | `tests/recovery-pools.test.ts` | 5 | Yes |
| Funds through the API | `tests/funds.test.ts` | 12 | Yes |
| Fund waterfall through the API: tier configuration with optimistic locking, distribution preview and apply, and two concurrent `apply` requests never both paying the same distribution | `tests/fund-waterfall.test.ts` | 11 | Yes |
...
| Updating a property: an omitted field is left alone, and an explicit `null` on a nullish field actually clears it rather than `COALESCE` silently keeping the old value | `tests/properties.test.ts` | 3 | Yes |
...
| Grid selection, clipboard, edit layer and column parsers | `apps/web/src/grid/grid.test.ts` | 42 | No |
...
| Entitlements: organization row, `/auth/me`, the `assumption_import` feature gate, and `updateEntitlements` syncing `features` to the new plan (not only `applyPlanDefaults`) | `tests/entitlements.test.ts` | 9 | Yes |
...
**262 browser tests in total**, for 1917 across the whole repository.
...
This paragraph used to claim property-based tests, performance/load tests and
worker tests had not started, and that the browser suite did not reach the
assumptions editor, scenario grid, versions, reports or the portfolio
builder. All of that is now false — see the property-based, performance,
load, concurrency and worker-tick rows above, and `e2e/assumptions.spec.ts`,
`e2e/scenarios.spec.ts`, `e2e/versions.spec.ts`, `e2e/reports.spec.ts` and
`e2e/portfolios.spec.ts` in the browser table. Left uncorrected long enough
that it stopped describing the codebase; corrected here rather than trusted.
